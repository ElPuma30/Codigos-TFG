...
def waypoints_distance(theta_n, theta_m):
    """
    Funcion:
    - Calcula la distancia entre dos vectores de 6 ángulos
...
def find_best_chains_by_start(waypoint_solutions):
    ...
    Input:
    - waypoint_solutions: lista (de tamaño n_waypoints) en la que cada elemento es otra lista con las
      distintas soluciones IK posibles (cada una es un array de 6 ángulos) para ese waypoint

    Parametros:
    - possible_solutions: lista de soluciones para cada waypoint
    - num_waypoints: cantidad total de waypoints
    - cost_min: lista de listas que indica el costo minimo para llegar a la j-esima solucion del waypoint i
    - prev: lista de listas que indica el predecesor de la j-esima solucion del waypoint i
    - start_chain: lista de listas para rastrear con que solucion del primer waypoint se inicio la cadena

    Retorna:
    - best_by_start: diccionario con las mejores cadenas para cada solucion del primer waypoint
    - global_best_chain: la cadena completa (lista de 6 ángulos por waypoint) con el menor costo de todas
    - global_best_cost: el costo total (suma de distancias angulares) de la cadena global más corta
    """
    ...
    # Inicializamos para el waypoint 0: costo 0 para cada solución y cada cadena empieza con sí misma.
    cost_min = []
    prev = []
    start_chain = []
    cost_min.append([0.0 for _ in range(possible_solutions)])
    prev.append([None for _ in range(possible_solutions)])
    start_chain.append([j for j in range(possible_solutions)])  # cada candidato se inicia con su propio índice

    # De waypoint 1 hasta el final.
    for i in range(1, num_waypoints):
        m_i = len(waypoint_solutions[i])
        cost_min_i = [float('inf')] * m_i
        prev_i = [None] * m_i
        start_i = [None] * m_i
        for j in range(m_i):
    ...
    # Para cada solución del primer waypoint, se extrae la mejor cadena que la parte.
    best_by_start = {}
